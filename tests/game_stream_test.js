...
test("GameStream status", function(){
    ...
    deepEqual(game_stream.status.duration,gs_status.duration,"The current duration is given by the KeyFrame: time is -2 and its counter is 0. That's because the action list doesn't have any other timestamped actions yet.");
...
test("Jump to time 50.", function(){
    ...
    equal(game_stream.status.waiting,gs_status.waiting,"We are not waiting because the stream has ended (and because the current time is not bigger than the current duration).");
});
